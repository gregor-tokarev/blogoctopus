<template>
  <div class="min-h-screen bg-background text-foreground p-4 flex flex-col items-center">
    <header class="w-full max-w-2xl mb-8">
      <h1 class="text-3xl font-bold text-center text-primary">Create New Post</h1>
    </header>
    ...
    <main class="w-full max-w-2xl p-6 bg-card rounded-lg shadow-md">
      <form @submit.prevent="handleSubmit">
        <div class="space-y-6">
          ...
          <div>
            <label for="content" class="block text-sm font-medium text-muted-foreground">Content</label>
            <Textarea
              id="content"
      ...
      <div v-if="submissionStatus" :class="['mt-6 p-4 rounded-md text-sm',
...
async function handleSubmit() {
  ...
  try {
    ...
    if (response) { // Simplified check, adjust as needed
      submissionStatus.value = { type: 'success', message: 'Post created and sent successfully!' };
      form.value.title = '';
      form.value.content = '';
    } else {
        ...
        submissionStatus.value = { type: 'error', message: 'Failed to create post. API returned an unexpected response.' };
    }
  } catch (error: any) {
