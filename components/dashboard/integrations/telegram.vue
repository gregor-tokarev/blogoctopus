...
const v$ = useVuelidate(
  {
    channelName: {
      ...
      validFormat: helpers.withMessage(
        "Формат канала должен быть @channel_name",
        helpers.regex(/^@[a-zA-Z][a-zA-Z0-9_]{3,30}$/),
      ),
    },
  },
  { channelName },
...
async function disconnectTelegram() {
  ...
  try {
    const result: any = await $fetch("/api/telegram/disconnect", {
      method: "post",
    });
  ...
  <UiAlertDialog :open="showDialog">
    ...
    <UiAlertDialogTrigger as="div">
      ...
      >
        ...
        <UiButton class="self-start mr-auto" v-else @click="showDialog = true">
    ...
    <UiAlertDialogContent>
          ...
          <div class="space-y-4 my-4">
              ...
              <div>
                ...
                <p>
                  ...
                  <span class="font-mono bg-muted px-1.5 py-0.5 rounded"
                    >@blogoctopus_integration_bot</span
                  >
                  в канал, где вы хотите публиковать сообщения, и сделайте его
                  администратором.
                </p>
              </div>
            </div>
            ...
            <div class="flex items-start gap-2">
              ...
              <div>
                ...
                <p>
                  Введите название вашего канала в поле ниже и нажмите
                  "Сохранить".
                </p>
              </div>
            </div>
            ...
            <form @submit.prevent="connectTelegram" class="mt-6">
              <div class="mb-4">
                <label for="channel-name" class="block text-sm font-medium mb-1"
                ...
                <div
                  ...
                  class="text-sm h-4 text-destructive mt-1"
                >
                  <span v-if="v$.channelName.required.$invalid"
                    >Имя канала обязательно</span
                  >
                  <span v-else-if="v$.channelName.validFormat.$invalid"
                    >Формат канала должен быть @channel_name</span
                  >
                </div>

                <p v-else class="text-xs text-muted-foreground mt-1 h-4">
                  Укажите имя канала, включая символ @
                </p>
              </div>
            </form>
          </div>
      ...
      <UiAlertDialogFooter>
        <UiAlertDialogCancel @click="showDialog = false"
          >Отмена</UiAlertDialogCancel
        >
        <UiAlertDialogAction
        ...
        >
          <LoaderCircle
            v-if="isConnecting"
            class="mr-0.5 size-4 animate-spin"
            aria-hidden="true"
          />
